...
public class Voter1_link_0_reference extends GenericReferenceActor<Voter.Voter1,Voter.Voter1> {
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, ReferenceNode node) {
		...
		ports.add(new PortEdgeLeft(node.getPorts().getPort().get(10), getSelf(), name2actor.get("switchRandom_29_junction"), this::check_constraint_11 , 10   ));
		ports.add(new PortEdgeRight(node.getPorts().getPort().get(11), getSelf(), name2actor.get("switchRandom_29_junction"), this::check_constraint_12 , 11   ));
	}	
