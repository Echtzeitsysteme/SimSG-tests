package groupVotersSim.rules.hipe.engine.actor.junction;
...
public class switchSame_37_junction extends GenericJunctionActor{
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, JunctionNode node) {
		...
		ports.add(new PortJunctionLeft(node.getPorts().getPort().get(0), getSelf(), name2actor.get("switchSame_36_nacjunction"), this::returnTrue , 0  , false ));
	}
	...
	protected void registerMatchForAttributeChanges(HMatch match) {
		...
		Collection<HMatch> voter1_4_Matches = voter1AttrMap.get(matchObjects[4]);
		if(voter1_4_Matches == null) {
			voter1_4_Matches = new LinkedList<>();
			voter1AttrMap.put(matchObjects[4], voter1_4_Matches);
		}
		
		voter1_4_Matches.add(match);
	...
	protected void deregisterMatchForAttributeChanges(Set<HMatch> matches, HMatch match) {
		...
		Collection<HMatch> matches_1 = voter1AttrMap.get(matchObjects[2]);
		if(matches_1 != null) {
			matches.remove(match);
		}
		Collection<HMatch> matches_2 = voter1AttrMap.get(matchObjects[4]);
		if(matches_2 != null) {
			matches.remove(match);
		}
	}
