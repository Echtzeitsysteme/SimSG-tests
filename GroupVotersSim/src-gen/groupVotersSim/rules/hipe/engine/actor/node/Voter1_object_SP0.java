...
public class Voter1_object_SP0 extends GenericObjectActor<GroupVoters.Voter1> {
	...
	protected void initializePorts(Map<String, ActorRef> name2actor, ObjectNode node) {
		...
		ports.add(new PortNodeLeft<GroupVoters.Voter1>(node.getPorts().getPort().get(1), getSelf(), name2actor.get("Voter1_member_2_reference"), this::check_constraint_4 , 1   ));
		ports.add(new PortNodeMatch<GroupVoters.Voter1>(node.getPorts().getPort().get(2), getSelf(), name2actor.get("voteFalse_production"), this::check_constraint_9 , 2   , node.getName() ));
	}
